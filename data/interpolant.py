...
from data import so3_utils, all_atom
...
class Interpolant:
    ...
    def _batch_ot(self, trans_0, trans_1, res_mask):
        # Find the optimal pairing between noisy (trans_0) and ground-truth (trans_1) structures within the batch.
        ...
        cost_matrix = torch.sum(
            torch.linalg.norm(aligned_nm_0 - aligned_nm_1, dim=-1) * batch_mask, dim=-1
        ) / torch.sum(batch_mask, dim=-1).clamp(min=1.0)
    ...
    def _corrupt_aatypes(self, aatypes_1, t, res_mask, diffuse_mask):
        ...
        # Mask the aatype with a probability of 1-t 
        ...
        #Substitue the aatype with random-sampled types
    ...
    def corrupt_batch(self, batch):
        ...
        if self._cfg.codesign_separate_t:
            # INPORTANT: The initial t is identical in all residues from one sequence
            # Then for one sequence, if diffuse mask is 0, the t for the residue is 1
            #u = torch.rand((num_batch,), device=self._device) #uniform distribution, (0,1]

            # 1. Choose where to forward fold or inverse fold (deprecated in our conditional generation model) ->ff_mask and if_mask
            # 2. (1) If we are forward folding, then cat_t should be 1
            #    (2) If we are inverse folding or codesign then cat_t should be uniform
            # 3. (1) If we are inverse folding, then structure_t should be 1
            #    (2) If we are forward folding or codesign then structure_t should be uniform
            '''
            ...
            '''
            structure_t = self.sample_t(num_batch)
            cat_t = self.sample_t(num_batch)
        ...
        #ligand features
        noisy_batch['ligand_pos'] = batch['ligand_pos']
        noisy_batch['ligand_elements'] = batch['ligand_elements']
        if "all_atom" in batch:
            noisy_batch["all_atom"] = batch["all_atom"]
        return noisy_batch
    ...
    def _regularize_step_probs(self, step_probs, aatypes_t):
        ...
        step_probs[
            torch.arange(batch_size, device=device).repeat_interleave(num_res), #[B,num_res,]
            torch.arange(num_res, device=device).repeat(batch_size), #[B*num_res,]
            aatypes_t.long().flatten() #[B, num_res] -> [B*num_res,]
        ] = 0.0
        step_probs[
    ...
    def _aatypes_euler_step(self, d_t, t, logits_1, aatypes_t):
        # Logits_1: model predicted sequence logits (NOT GROUND TRUTH) [B, num_res, 21]
        # aatypes_t: aatype at t time (discrete index) 
        # S = 21
        # du.MASK_TOKEN_INDEX is 21 during training and 20 during inference.
        ...
        if self._aatypes_cfg.interpolant_type == "masking":
            ...
            logits_1[:, :, du.MASK_TOKEN_INDEX] = -1e4

            # Using softmax to turn logits into probability
            # Enhanced confidence at low temperatures, smooth distribution at high temperatures
            # THIS STEP IS REGULARIZED (WITH MASKED POSITION ZERO PROBABILITY)
            pt_x1_probs = F.softmax(logits_1 / self._aatypes_cfg.temp, dim=-1) # (B, D, S) 
            
            # Mask position in current sequence
            aatypes_t_is_mask = (aatypes_t == du.MASK_TOKEN_INDEX).view(batch_size, num_res, 1).float() #(B, D, 1)
    ...
    # Compute how many positions to unmask at this time step, then fill the [MASK] positions with highest log probability.
    # 1. Probabilities -> 2. confidence -> 3. rank [MASK] positions -> 4. sample fill count -> 5. fill -> 6. randomly remask a few.
    ...
    def sample(
        ...
        batch.update(condition) # add ligand_pos, ligand_elements
        if predict_sidechain:
            assert len(sidechain_dict)>0, "sidechain dict must be provided when predicting side chain"
            batch.update(sidechain_dict)
        if trans_1 is None:
        ...
        trans_t_1, rotmats_t_1, aatypes_t_1 = trans_0, rotmats_0, aatypes_0 #Noise
        ...
        if predict_sidechain:
            # convert 14 to 37
            atom37, _ = atom14_to_atom37(atom14_data = model_out["all_atom"]["positions"],
                                         aatype = pred_aatypes_1)
            # adjust oxygen coordinate
            ...
            pred_atom37 = torch.stack(pred_atom37)
        else:
            #pred_atom37 = frames_to_atom37(pred_trans_1, pred_rotmats_1)
            pred_atom37 = all_atom.atom37_from_trans_rot(pred_trans_1, pred_rotmats_1, res_mask, pred_aatypes_1).detach().cpu()

        clean_traj.append((pred_atom37, pred_aatypes_1.detach().cpu()))
        prot_traj.append((pred_atom37, pred_aatypes_1.detach().cpu()))
        return prot_traj, clean_traj, model_out #last timestep model out
