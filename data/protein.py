...
class Protein:
  ...
  # Insertion Code for antibodys and others
...
def _chain_end(atom_index, end_resname, chain_name, residue_index, insertion_code=None) -> str:
  chain_end = 'TER'
  if insertion_code:
      return (f'{chain_end:<6}{atom_index:>5}      {end_resname:>3} '
            f'{chain_name:>1}{residue_index:>4}{insertion_code:>1}')
  else:
      return (f'{chain_end:<6}{atom_index:>5}      {end_resname:>3} '
...
def to_pdb(prot: Protein, model=1, add_end=True) -> str:
  ...
  insertion_code = prot.insertion_code
  ...
  for i in range(aatype.shape[0]):
    ...
    if last_chain_index != chain_index[i]:
      pdb_lines.append(_chain_end(
          ...
          residue_index[i - 1], insertion_code[i - 1] if insertion_code is not None else None))
  ...
  pdb_lines.append(_chain_end(atom_index, res_1to3(aatype[-1]),
                              chain_ids[chain_index[-1]], residue_index[-1], icode))
