...
from Bio import PDB
from data import protein, residue_constants, parsers
...
def parse_chain_feats(chain_feats, scale_factor=1., center=True):
    bb_mask = chain_feats['atom_mask'][:, CA_IDX]
    bb_pos = chain_feats['atom_positions'][:, CA_IDX] #(num_res, 3)
    if center:
        bb_center = np.sum(bb_pos, axis=0) / (np.sum(bb_mask) + 1e-5) #(3,)
...
def align_structures(
    ...
    u_t_fp32 = u_t.to(torch.float32)
    sign_correction = torch.sign(torch.linalg.det(torch.bmm(v_fp32, u_t_fp32).to(torch.float32)))
    sign_correction = sign_correction.to(v.dtype)  
...
def find_pocket_range(diffuse_mask, old_res_index, chain_index):
    assert diffuse_mask.shape == old_res_index.shape == chain_index.shape, \
        "All input arrays must have the same shape"
    old_res_idx = old_res_index.copy()
    chain_idx = np.array(list(protein.PDB_CHAIN_IDS))[chain_index]
    indices = [i for i, val in enumerate(diffuse_mask) if val == 1]
    if not indices:
        return ""
    ...
    for idx in indices:
        chain = chain_idx[idx]
        residue = old_res_idx[idx]
        if chain not in chain_groups:
            chain_groups[chain] = []
        chain_groups[chain].append((residue, idx))
    
    # extract chain residue ids
    chain_segments = {}
    for chain, positions in chain_groups.items():
        positions.sort(key=lambda x: x[0])
        
        sorted_indices = [pos[1] for pos in positions]
        ...
        for i in range(1, len(sorted_indices)):
            ...
            if current_idx == prev_idx + 1:
                ...
                if start_res == prev_res:
                    segments.append(str(start_res))
                else:
                    segments.append(f"{start_res}-{prev_res}")
                start_idx = current_idx
                start_res = current_res
                prev_idx = current_idx
                prev_res = current_res
    ...
    result_parts = []
    for chain in sorted(chain_segments.keys()): 
        segments = chain_segments[chain]
        if segments:
            result_parts.append(f"{chain}.{','.join(segments)}")
    
    return "_".join(result_parts)

def check_atom_clash_contact(chain1: Chain, chain2: Chain, clash_distance=1.0, contact_distance=5.5):

    coords1 = np.array([atom.coord for atom in chain1.get_atoms()])
    coords2 = np.array([atom.coord for atom in chain2.get_atoms()])
    ...
    return np.any(distances < clash_distance), np.any(distances < contact_distance)


def merge_PDB(ligand_pdb:str, receptor_pdb:str, save_dir:str, clash_distance=1.0, contact_distance = 5.5):
    
    parser = PDBParser(QUIET=True)
    ligand_struct = parser.get_structure("ligand_struct", ligand_pdb)
    ligand_chain_list = list(ligand_struct.get_chains())
    assert len(ligand_chain_list) == 1, f"Ligand chain number must be one,got {len(ligand_chain_list)}"
    ligand_chain = ligand_chain_list[0]
    existing_chains = {ligand_chain.id}
    receptor_struct = parser.get_structure("receptor_struct", receptor_pdb)
    for chain in receptor_struct.get_chains():
        has_clash, has_contact = check_atom_clash_contact(chain, ligand_chain, clash_distance, contact_distance)
        if not has_clash and has_contact:
            new_chain = chain.copy()
            new_chain.id = chr(ord(max(existing_chains))+1)
            existing_chains.add(new_chain.id)
            ligand_struct[0].add(new_chain)  # Add the ligand chain to the model.
        else:
            continue
    
    assert len(list(ligand_struct.get_chains())) >= 1, "Final merged chain number must greater than 1"
    io = PDBIO()
    io.set_structure(ligand_struct)
    path = os.path.join(save_dir, os.path.basename(ligand_pdb))
    io.save(path)