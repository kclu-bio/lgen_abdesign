...
def compute_fape(
    ...
    """
    # [*, N_frames, N_pts, 3]
    # Project global coordinates into the local coordinate system.
...
def sidechain_loss(
    ...
    **kwargs,
    ...
    # Flatten [batch, residues, rigid_groups, 4, 4] to [batch, residues * rigid_groups, 4, 4].
...
def fape_loss(
    ...
    sc_loss = sidechain_loss(
        out["sm"]["sidechain_frames"], #predicted frame (8)
        out["sm"]["positions"], #predicted position [*, N, 14, 3]
...
def compute_renamed_ground_truth(
    ...
    lddt = torch.sqrt(eps + (pred_dists - gt_dists) ** 2) # Difference from the original naming.
    alt_lddt = torch.sqrt(eps + (pred_dists - alt_gt_dists) ** 2) # Difference from the alternative naming.

    atom14_gt_exists = batch["atom14_gt_exists"]
    atom14_atom_is_ambiguous = batch["atom14_atom_is_ambiguous"]
    mask = (
        atom14_gt_exists[..., None, :, None] # Ensure the first atom exists.
        * atom14_atom_is_ambiguous[..., None, :, None] # The first atom must have ambiguous naming.
        * atom14_gt_exists[..., None, :, None, :]  # Ensure the second atom exists.
        * (1.0 - atom14_atom_is_ambiguous[..., None, :, None, :]) # The second atom must have unambiguous naming.
    )
    # Thus mask[..., i, a, j, b] = 1.0 exactly when:
    # atom a in residue i exists and has ambiguous naming, and
    # atom b in residue j exists and has unambiguous naming.

    # For each residue, sum distance differences between all ambiguous and unambiguous atoms.
    per_res_lddt = torch.sum(mask * lddt, dim=(-1, -2, -3))  #[*, N]
    alt_per_res_lddt = torch.sum(mask * alt_lddt, dim=(-1, -2, -3))

    fp_type = atom14_pred_positions.dtype
    alt_naming_is_better = (alt_per_res_lddt < per_res_lddt).type(fp_type) #[*, N]
    # A value of 1 means this residue uses alternative naming.
