...
def to_tensor(data):
    """Convert input to a gradient-free CPU tensor."""
    if isinstance(data, torch.Tensor):
        return data.detach().clone().cpu()
    else:
        # Convert NumPy arrays directly.
        return torch.from_numpy(np.array(data)).cpu()

def calc_rmsd(mask, sample_bb_pos, folded_bb_pos):
    """
        Args:
            mask:
                [*, N * n] tensor/np.array
            sample_bb_pos:
                [*, N * n, 3] tensor/np.array
            folded_bb_pos:
                [*, N * n, 3] tensor/np.array
        Returns:
            rmsd [*,]
    """
    mask = to_tensor(mask)
    sample_bb_pos = to_tensor(sample_bb_pos)
    folded_bb_pos = to_tensor(folded_bb_pos)
...
def process_folded_outputs(sample_path, folded_output, mask: torch.Tensor, true_bb_pos=None):
    # compare sample path structure and folded_output structure RMSD
    results = {
        'header': [],
        'sequence': [],
        'bb_rmsd': [],
    ...
    if true_bb_pos is not None:
        results['bb_rmsd_to_gt'] = []
        results['fold_model_bb_rmsd_to_gt'] = []
    ...
    for _, row in folded_output.iterrows():
        ...
        if true_bb_pos is not None:
            bb_rmsd_to_gt = calc_bb_rmsd(mask, sample_bb_pos, true_bb_pos)
            results['bb_rmsd_to_gt'].append(bb_rmsd_to_gt)
            fold_model_bb_rmsd_to_gt = calc_bb_rmsd(mask, folded_bb_pos, true_bb_pos)
            results['fold_model_bb_rmsd_to_gt'].append(fold_model_bb_rmsd_to_gt)
        bb_rmsd = calc_bb_rmsd(mask, sample_bb_pos, folded_bb_pos)
        results['bb_rmsd'].append(bb_rmsd)
        results['folded_path'].append(row.folded_path)
        results['header'].append(row.header)
        results['sequence'].append(seq)
        results['mean_plddt'].append(row.plddt)
    results = pd.DataFrame(results)
    results['sample_path'] = sample_path
    return results
...
def calc_aar(pred_aa:np.array, 
    ...
    aa: (B, N)
    """
    if pred_aa.ndim == 3: #Logits
        pred_aa = np.argmax(pred_aa, axis=-1)  # (B, N)
    elif pred_aa.ndim == 2: #Index
        pass
    else:
        raise ValueError(f'Unrecognized pred_aa shape {pred_aa.shape}')
    correct = (pred_aa == aa)  # (B, N)
    masked_correct = correct.astype(float) * mask.astype(float)  # (B, N)
    correct_sum = np.sum(masked_correct, axis=-1)  # (B,)
    valid_sum = np.sum(mask, axis=-1).astype(float)  # (B,)
    coverage = correct_sum / (valid_sum + 1e-8)
